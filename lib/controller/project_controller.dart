...
class ProjectController extends GetxController {
  ...
  int get totalPages => (allProjects.where(_matchesFilter).length / itemsPerPage.value).ceil();

  @override
  void onInit() {
    super.onInit();
    fetchProjects();
    debounce(searchQuery, (_) => filterAndSortProjects(), time: const Duration(milliseconds: 400));
  }

  void fetchProjects() {
    allProjects.value = [
      Project(id: '1', name: 'پروژه برج میلاد', clientName: 'شهرداری تهران', supervisorName: 'مهندس رضایی', floorCount: 20, contractDate: '2023-05-10', mainTestType: 'مقاومت فشاری', municipalDistrict: 'منطقه ۲'),
      Project(id: '2', name: 'پروژه تجاری ایران‌مال', clientName: 'گروه صنعتی تات', supervisorName: 'مهندس زمانی', floorCount: 8, contractDate: '2024-02-20', mainTestType: 'چکش اشمیت', municipalDistrict: 'منطقه ۲۲'),
      Project(id: '3', name: 'پروژه مسکونی الهیه', clientName: 'شرکت آرمان', supervisorName: 'مهندس اکبری', floorCount: 15, contractDate: '2025-08-01', mainTestType: 'مقاومت فشاری', municipalDistrict: 'منطقه ۱'),
    ];
    filterAndSortProjects();
  }
  
  void updateSearchQuery(String query) => searchQuery.value = query;
  void changeItemsPerPage(int size) { itemsPerPage.value = size; filterAndSortProjects(); }
  void goToPage(int page) { if(page > 0 && page <= totalPages) { currentPage.value = page; filterAndSortProjects(); } }
  ...
  bool _matchesFilter(Project p) {
    ...
    return p.name.toLowerCase().contains(query) || p.clientName.toLowerCase().contains(query) || p.supervisorName.toLowerCase().contains(query);
  }
  ...
  void filterAndSortProjects() {
    ...
    switch (sortColumnIndex.value) {
      case 0: filtered.sort((a, b) => a.name.compareTo(b.name) * compare); break;
      case 1: filtered.sort((a, b) => a.contractDate.compareTo(b.contractDate) * compare); break;
      case 2: filtered.sort((a, b) => a.clientName.compareTo(b.clientName) * compare); break;
      case 3: filtered.sort((a, b) => a.supervisorName.compareTo(b.supervisorName) * compare); break;
      case 4: filtered.sort((a, b) => a.municipalDistrict.compareTo(b.municipalDistrict) * compare); break;
      case 5: filtered.sort((a, b) => a.floorCount.compareTo(b.floorCount) * compare); break;
      case 6: filtered.sort((a, b) => a.mainTestType.compareTo(b.mainTestType) * compare); break;
    }
    ...
    Get.defaultDialog(
      title: "تایید حذف", middleText: "آیا از حذف این پروژه اطمینان دارید؟",
      textConfirm: "بله، حذف کن", textCancel: "انصراف",
      confirmTextColor: Colors.white, buttonColor: Colors.red.shade600,
      onConfirm: () {
